...
//
// Servers that require a cookie (confirmed by reading PocketMine-MP's RakLib
// source, since go-raknet's version of this packet didn't match) detect its
// presence by the packet's remaining length rather than a flag byte, and
// expect cookie(4) + a 1-byte "encryption challenge" placeholder (5 bytes
// total), not just the bare 4-byte cookie. This client never uses libcat
// security, so the challenge byte is always 0.
