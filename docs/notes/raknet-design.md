...
The initial plan for this milestone flagged several byte-layout details as unverified (frame length in bits vs. bytes, field endianness, address XOR encoding, system-address list count, disconnect packet ID). All were resolved by reading a real implementation and confirming empirically against a real server, rather than guessing:
...
- **Anti-amplification cookie**: `OpenConnectionReply1` may carry a `HasSecurity` flag + 4-byte cookie; if present, the client must echo it back in `OpenConnectionRequest2`. The cookie region turned out to be 5 bytes, not 4: a cookie (4 bytes) *plus* a 1-byte "encryption challenge" placeholder immediately after it, detected by the packet's remaining length rather than a flag. Sending only the bare 4-byte cookie desyncs every field after it by one byte - this was the actual bug caught during implementation (see below). Confirmed empirically against real BDS: the handshake reaches `Connected` against it.

## A real bug this caught

The first implementation of `OpenConnectionReply1.Decode`/`OpenConnectionReply2.Decode` never skipped the 16-byte magic prefix before reading fields, so `ServerGuid`/`ServerHasSecurity`/`Cookie` were read out of the magic bytes themselves. The tell: the decoded cookie value was `0xFDFDFD12`, visibly a slice of the magic sequence (`...FD FD FD FD 12 34 56 78`), not a real cookie. Caught by hex-dumping the actual outgoing `OpenConnectionRequest2` packet and cross-checking field-by-field, not by static reasoning. Fixed alongside the 5-byte cookie-region bug above; see git history for both.

## Confirmed empirically: an early test server enforces a login-completion timeout

One of the plan's open questions was whether a RakNet-only session (no Bedrock `Login` packet ever sent) survives indefinitely. Against an earlier test server (since retired - see [`.skills/bedrock-integration-testing`](../../.skills/bedrock-integration-testing/SKILL.md)) it did not: the session closed exactly 10 seconds after opening, logged as a login timeout, regardless of a healthy `ConnectedPing`/`ConnectedPong` exchange in the meantime. Reproduced twice with matching timing. Whether real BDS behaves the same way is unconfirmed. This is expected either way, not a bug in this slice - see the Definition of Done in the milestone plan: this slice's bar is reaching `Connected` and proving at least one keep-alive round trip, not staying up forever. Indefinite idling is Milestone 1's *full* bar (RakNet + partial Bedrock login together), which is out of scope here.
